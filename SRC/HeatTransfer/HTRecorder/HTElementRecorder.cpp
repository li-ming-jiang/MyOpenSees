...
HTElementRecorder::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  if (theChannel.isDatastore() == 1) {
    opserr << "HTElementRecorder::sendSelf() - does not send data to a datastore\n";
    return -1;
  }
  ...
  if (theChannel.sendID(0, commitTag, idData) < 0) {
    opserr << "HTElementRecorder::sendSelf() - failed to send idData\n";
    return -1;
  }
  ...
  if (theChannel.sendVector(0, commitTag, dData) < 0) {
    opserr << "HTElementRecorder::sendSelf() - failed to send dData\n";
    return -1;
  }
  ...
  if (eleID != 0)
    if (theChannel.sendID(0, commitTag, *eleID) < 0) {
      opserr << "HTElementRecorder::sendSelf() - failed to send eleID\n";
      return -1;
    }
  ...
  if (dof != 0)
    if (theChannel.sendID(0, commitTag, *dof) < 0) {
      opserr << "HTElementRecorder::sendSelf() - failed to send dof\n";
      return -1;
    }
  ...
  if (msgLength ==  0) {
    opserr << "HTElementRecorder::sendSelf() - no data to send!!\n";
    return -1;
  }

  char *allResponseArgs = new char[msgLength];
  if (allResponseArgs == 0) {
    opserr << "HTElementRecorder::sendSelf() - out of memory\n";
    return -1;
  }
  ...
  if (theChannel.sendMsg(0, commitTag, theMessage) < 0) {
    opserr << "HTElementRecorder::sendSelf() - failed to send message\n";
    return -1;
  }
  ...
  if (theOutputHandler == 0 || theOutputHandler->sendSelf(commitTag, theChannel) < 0) {
    opserr << "HTElementRecorder::sendSelf() - failed to send the DataOutputHandler\n";
    return -1;
  }
		 ...
		 FEM_ObjectBroker &theBroker)
{
  ...
  if (theChannel.isDatastore() == 1) {
    opserr << "HTElementRecorder::recvSelf() - does not recv data to a datastore\n";
    return -1;
  }
  ...
  if (theChannel.recvID(0, commitTag, idData) < 0) {
    opserr << "HTElementRecorder::recvSelf() - failed to recv idData\n";
    return -1;
  }
  ...
  if (eleSize != 0) {
    ...
    if (eleID == 0) {
      opserr << "HTElementRecorder::recvSelf() - failed to create eleID\n";
      return -1;
    }
    if (theChannel.recvID(0, commitTag, *eleID) < 0) {
      opserr << "HTElementRecorder::recvSelf() - failed to recv eleOD\n";
      return -1;
    }
  }
  ...
  if (numDOF != 0) {
    dof = new ID(numDOF);
    if (dof == 0) {
      opserr << "HTElementRecorder::recvSelf() - failed to create dof\n";
      return -1;
    }
    if (theChannel.recvID(0, commitTag, *dof) < 0) {
      opserr << "HTElementRecorder::recvSelf() - failed to recv dof\n";
      return -1;
    }
  }
  ...
  if (msgLength == 0) {
    opserr << "HTElementRecorder::recvSelf() - 0 sized string for responses\n";
    return -1;
  }

  char *allResponseArgs = new char[msgLength];
  if (allResponseArgs == 0) {
    opserr << "HTElementRecorder::recvSelf() - out of memory\n";
    return -1;
  }

  Message theMessage(allResponseArgs, msgLength);
  if (theChannel.recvMsg(0, commitTag, theMessage) < 0) {
    opserr << "HTElementRecorder::recvSelf() - failed to recv message\n";
    return -1;
  }
