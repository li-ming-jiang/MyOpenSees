...
OPS_TimberECThermal(void)
{
    ...
    if (numData != 7) {
        opserr << "Invalid #args, want: uniaxialMaterial TimberECThermal " << iData[0] << "fpc? epsc0? fpcu? epscu? rat? ft? Ets?\n";
        return 0;
    }
    ...
    theMaterial = new TimberECThermal(iData[0], dData[0], dData[1], dData[2], dData[3], dData[4], dData[5], dData[6]);
...
{
    TimberECThermal* theCopy = new TimberECThermal(this->getTag(), fc, epsc0, fcu, epscu, rat, ft, Ets);
...
{
    s << "TimberECThermal:(strain, stress, tangent) " << eps << " " << sig << " " << e << endln;
}


void
TimberECThermal::Tens_Envlp(double epsc, double& sigc, double& Ect)
{
    /*-----------------------------------------------------------------------
...
void
TimberECThermal::Compr_Envlp(double epsc, double& sigc, double& Ect)
{
    /*-----------------------------------------------------------------------
