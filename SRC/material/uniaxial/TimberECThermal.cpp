...
OPS_SteelECThermal(void)
{
	...
	if (OPS_GetNumRemainingInputArgs() == 3 || OPS_GetNumRemainingInputArgs() == 7)
	{
		...
		if (strcmp(typeChar, "EC3") == 0) {
			...
			opserr << "WARNING invalid material type for uniaxialMaterial TimberECThermal " << iData[0] << endln;
			return 0;
		}
	...
	if (numData != 2 && numData != 6) {
		opserr << "Invalid #args, want: uniaxialMaterial TimberECThermal " << iData[0] << " fy? E? b? <a1? a2? a3? a4?>>" << endln;
		return 0;
	}
	...
	theMaterial = new TimberECThermal(iData[0], iData[1], dData[0], dData[1],
...
TimberECThermal::TimberECThermal
...
TimberECThermal::TimberECThermal() :UniaxialMaterial(0, MAT_TAG_SteelECThermal),
...
int TimberECThermal::setTrial(double strain, double& stress, double& tangent, double strainRate)
{
	// Reset history variables to last converged state
	TminStrain = CminStrain;
	TmaxStrain = CmaxStrain;
...
void TimberECThermal::determineTrialState(double dStrain)
{
	...
	else if (fabsTstrain <= EpsiU)
	{
		...
		//opserr<<"Error: Stiffness of TimberECThermal is negative"<<endln;
	...
	else
	{
		Tstress = 1E-10;
		Ttangent = 1E-10;
		//opserr<<"Error: Trial strain ("<< Tstrain<<") for TimberECThermal is invalid"<<endln;;
	}
...
void TimberECThermal::detectLoadReversal(double dStrain)
{
...
double TimberECThermal::getStrain()
{
	return Tstrain;
}

double TimberECThermal::getStress()
{
	return Tstress;
}

double TimberECThermal::getTangent()
{
	return Ttangent;
}

double
TimberECThermal::getThermalElongation(void)
{
...
double
TimberECThermal::getElongTangent(double TempT, double& ET, double& Elong, double TempTmax)
{
	if (TempT < -100 || TempT>1200)
		opserr << "WARNING TimberECThermal received an invalid temperature: " << TempT << endln;
	...
	if (typeTag == 0 || typeTag == 3) {
		...
		opserr << "WARNING TimberECThermal received an invalid typeTag: " << typeTag << endln;
	...
	for (int i = 0; i < 13; i++) {
		if (TempT <= 80 + 100 * i)
		{
			if (i == 0) {
				...
				opserr << "Warning:The temperature " << TempT << " for TimberECThermal is out of range\n";
				return -1;
			}
			else {
...
UniaxialMaterial* TimberECThermal::getCopy()
{
	TimberECThermal* theCopy = new TimberECThermal(this->getTag(), typeTag, fy, E0,
		a1, a2, a3, a4);
...
int TimberECThermal::sendSelf(int commitTag, Channel& theChannel)
{
	...
	if (res < 0)
		opserr << "TimberECThermal::sendSelf() - failed to send data\n";

	return res;
}

int TimberECThermal::recvSelf(int commitTag, Channel& theChannel,
	FEM_ObjectBroker& theBroker)
{
	...
	if (res < 0) {
		opserr << "TimberECThermal::recvSelf() - failed to receive data\n";
		this->setTag(0);
	}
	else {
...
{
	s << "TimberECThermal tag: " << this->getTag() << endln;
	s << "typeTag: " << typeTag << " ";
	s << "  fy: " << fy << " ";
	s << "  E0: " << E0 << " ";
	s << "  a1: " << a1 << " ";
...
// AddingSensitivity:BEGIN ///////////////////////////////////
int
TimberECThermal::setParameter(const char** argv, int argc, Parameter& param)
{
...
int
TimberECThermal::updateParameter(int parameterID, Information& info)
{
	switch (parameterID) {
	case -1:
		return -1;
...
int
TimberECThermal::activateParameter(int passedParameterID)
{
...
double
TimberECThermal::getStressSensitivity(int gradIndex, bool conditional)
{
...
double
TimberECThermal::getInitialTangentSensitivity(int gradIndex)
{
...
int
TimberECThermal::commitSensitivity(double TstrainSensitivity, int gradIndex, int numGrads)
{
	if (SHVs == 0) {
...
int TimberECThermal::setTrialStrain(double strain, double strainRate)
{
...
int
TimberECThermal::getVariable(const char* variable, Information& info)
{
